<?php
 ...
 if(isset($_GET['function']) && !empty($_GET['function'])){
  function lista(){
   ...
   $html_code = "<titulo>Listado de asistencia</titulo><br>";
   $html_code.="<body><form method=post action=grabar.php>";
   $html_code.="<table style=text-align:left; width:100%; border=1 cellpadding=0 cellspacing=0>";
   $html_code.="<tbody>";
   ...
   while($var = mysql_fetch_row($query_id1)){
    ...
    $html_code.="<tr>";
    $html_code.="<input type=hidden name=id_curso value=$id_curso>";
    $html_code.="<input type=hidden name=id_asistente$renglon value=$id_asistente>";
    $html_code.="<td>$cliente<input type=hidden name=cliente$renglon value='$cliente'></td>";
    $html_code.="<td>Otro Asistente<br>Nombre<input type=text size=25 name=otro$renglon></td>";
    $html_code.="<td><input type=text size=25 name=comentarios$renglon></td>";
   ...
   $html_code.="</tbody>";
   $html_code.="</table><br>";
   $html_code.="<input type=hidden name=renglon value=$renglon>";
   $html_code.="<input value=Guardar! type=submit>";
   $html_code.="<input value=Borrar datos! type=reset>";
   $html_code.="</form></body>";
...
<?
 //Leer los cursos abiertos desde la base y poblar el select para el html
 ...
 //Leer el id_curso, la fecha y el lugar para publicarlos en el select, cursos solo abiertos 1
 $strsql = "SELECT * FROM curso_encabezado WHERE estado = 1";
 ...
 if (mysql_num_rows($query_id) > 0) {
  ...
  while($var = mysql_fetch_row($query_id)){
   ...
   $lugar = $var[3];
   $option = "Curso ".$_SESSION['$id_curso']."=>".$_SESSION['$fecha'].".".$lugar;
