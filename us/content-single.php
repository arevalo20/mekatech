...
?>
<article id="post-<?php the_ID(); ?>" class="post type-post status-publish<?php if ('style-2' != $single_style) : echo ' hentry';
																																					endif; ?>">
	...
	<?php if ('style-2' != $single_style) : ?>
		...
		<?php if (has_post_format('link') || has_post_format('quote')) : ?>
			<div class="standard-post-special post-image<?php if (has_post_format('quote')) : ?> penci-special-format-quote<?php endif; ?><?php if (!has_post_thumbnail() || get_theme_mod('penci_standard_thumbnail')) : echo ' no-thumbnail';
																																																																				endif; ?>">
						...
						<p class="dt-special">
							<?php
									if (has_post_format('quote')) {
										...
										if (!empty($dt_content)) : echo sanitize_text_field($dt_content);
										endif;
									} else {
		...
		<?php elseif (has_post_format('audio')) : ?>
			...
			<div class="standard-post-image post-image audio<?php if (!has_post_thumbnail() || get_theme_mod('penci_post_thumb')) : echo ' no-thumbnail';
																													endif; ?>">
		...
		<?php else : ?>
			...
			<center>
				<div>
					<!-- /1568694929415-4/dpf_EdoMex -->
					<div id="div-gpt-ad-1568694929415-4">
						<script>
							googletag.cmd.push(function() {
								googletag.display('div-gpt-ad-1568694929415-4');
							});
						</script>
					</div>
					<!-- /1568694929415-4/dpf_EdoMex -->
			...
			<!-- <a target="_blank" style="display: block; max-width: 100%; margin-left: auto; margin-right: auto;">
				<img alt="Puebla Honesta" src="https://www.lanetanoticias.com/wp-content/uploads/2019/08/utliles-830x130.jpg">
			</a> -->
			<br />
	...
	<?php if (get_theme_mod('penci_move_title_bellow') && 'style-2' != $single_style) : ?>
		...
		<a target="_blank" href="https://wa.me/522223433989" style="display: block; max-width: 100%; margin-left: auto; margin-right: auto;">
			<img alt="Facebook-LNN" src="https://www.lanetanoticias.com/wp-content/uploads/2019/10/output_aWdj2q.gif">
		</a>
		<br />
		...
		<div class="header-standard header-classic single-header">
			...
			<!-- Anuncio video -->
			<!-- <iframe src="https://player.vimeo.com/video/337319966?autoplay=1&loop=1&autopause=0" width="830" height="130" frameborder="0" allow="autoplay"> -->
			<!-- </iframe> -->
			<!-- End Anuncio video -->

			<center>
				<!-- /1568689114056/dfp_LNN_sedena -->
				<div id='div-gpt-ad-1568689114056-0' style='width: 300px; height: 250px;'>
					<script>
						googletag.cmd.push(function() {
							googletag.display('div-gpt-ad-1568689114056-0');
						});
					</script>
				</div>
				<!-- 21686962517 -->
	...
	<?php if (!get_theme_mod('penci_single_meta_comment') || !get_theme_mod('penci_post_share')) : ?>
		<div class="tags-share-box center-box">
			...
			<?php if (!get_theme_mod('penci_single_meta_comment')) : ?>
				<span class="single-comment-o<?php if (get_theme_mod('penci_post_share')) : echo ' hide-comments-o';
																					endif; ?>"><i class="fa fa-comment-o"></i><?php comments_number('0 ' . penci_get_setting('penci_trans_comment'), '1 ' . penci_get_setting('penci_trans_comment'), '% ' . penci_get_setting('penci_trans_comments')); ?></span>
			<?php endif; ?>

			<?php if (!get_theme_mod('penci_post_share')) : ?>
				<div class="post-share<?php if (get_theme_mod('penci__hide_share_plike')) : echo ' hide-like-count';
																	endif; ?>">
					<?php if (!get_theme_mod('penci__hide_share_plike')) : echo penci_single_getPostLikeLink(get_the_ID());
							endif; ?>
